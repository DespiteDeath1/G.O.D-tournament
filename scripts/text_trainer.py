#!/usr/bin/env python3
"""
Optimized text model training script for G.O.D. Tournament
"""
...
def get_optimized_config(task_type, model, dataset_size=None):
    """Get optimized hyperparameters based on task type and model"""
    
    # Базовые оптимизированные параметры
    optimized_params = {
        "gradient_accumulation_steps": 8,  # Увеличено для стабильности
        "micro_batch_size": 4,  # Оптимально для большинства GPU
        "num_epochs": 1,
        "optimizer": "adamw_torch",  # Более стабильный чем 8bit версии
        "lr_scheduler": "cosine",
        "warmup_steps": 100,  # Увеличено для лучшей стабилизации
        "eval_steps": 50,
        "save_steps": 100,
        "logging_steps": 10,
        "gradient_checkpointing": True,  # Всегда включено для экономии памяти
        "bf16": True,  # Для H200
        "tf32": True,  # Ускорение на новых GPU
        "flash_attention": True,
        "sample_packing": True,  # Эффективнее использует батчи
    ...
    lora_params = {
        "adapter": "lora",
        "lora_r": 128,  # Увеличено для лучшего качества
        "lora_alpha": 256,  # 2x от rank
        "lora_dropout": 0.05,
        "lora_target_linear": True,
        "lora_modules_to_save": ["embed_tokens", "lm_head"],  # Сохраняем важные слои
    }
    ...
    if task_type == TaskType.INSTRUCTTEXTTASK.value:
        optimized_params.update({
            "learning_rate": 2e-4,
            "num_epochs": 2,  # Больше эпох для инструкций
        })
    elif task_type == TaskType.DPOTASK.value:
        optimized_params.update({
            "learning_rate": 5e-5,  # Ниже для DPO
            "num_epochs": 1,
            "eval_steps": 25,
            "save_steps": 50,
        })
        lora_params["lora_r"] = 64  # Меньше для DPO
        lora_params["lora_alpha"] = 128
    ...
    # Оптимизация по размеру модели
    model_size = model.lower()
    if "70b" in model_size:
        optimized_params["micro_batch_size"] = 1
        optimized_params["gradient_accumulation_steps"] = 16
        lora_params["lora_r"] = 64
        lora_params["lora_alpha"] = 128
    elif "7b" in model_size or "8b" in model_size:
        optimized_params["micro_batch_size"] = 4
        optimized_params["gradient_accumulation_steps"] = 8
    elif "1b" in model_size or "3b" in model_size:
        optimized_params["micro_batch_size"] = 8
        optimized_params["gradient_accumulation_steps"] = 4
        optimized_params["learning_rate"] *= 2  # Можем увеличить LR для маленьких моделей
    
    return {**optimized_params, **lora_params}


def create_config(task_id, model, dataset, dataset_type, file_format, output_dir, expected_repo_name=None,
                huggingface_username=None, huggingface_token=None, disable_upload=True, task_type=None):
    """Create the axolotl config file with optimized settings."""
    ...
    optimized_params = get_optimized_config(task_type, model)
...
def run_training(config_path):
    print(f"Starting optimized training with config: {config_path}", flush=True)
    ...
    training_env["PYTORCH_CUDA_ALLOC_CONF"] = "max_split_size_mb:512"  # Оптимизация памяти
    training_env["CUDA_LAUNCH_BLOCKING"] = "0"  # Асинхронные операции

    training_command = [
        "accelerate", "launch",
        "--mixed_precision", "bf16",  # Явно указываем precision
        "--num_processes", "1",  # Для одной GPU
        "--num_machines", "1",
        "--dynamo_backend", "inductor",  # Torch compile backend
        "-m", "axolotl.cli.train",
        config_path
    ]

    try:
        print("Starting training subprocess with optimizations...\n", flush=True)
...
async def main():
    print("---STARTING OPTIMIZED TEXT TRAINING SCRIPT---", flush=True)
    parser = argparse.ArgumentParser(description="Optimized Text Model Training Script")
